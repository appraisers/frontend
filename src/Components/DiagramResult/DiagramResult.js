...
function CompassCenter({ origin }) {
  const circleStyle = {
    stroke: centerColor.base,
    strokeWidth: 2,
    fill: centerColor.highlight,
  };
  ...
  return (
    ...
    >
      <Table size="small" style={{ maxWidth: 600 }}>
        <TableHead>
          <TableRow>
            <TableCell>Название направления</TableCell>
            <TableCell align="center">Заинтересованность</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {rows.map((row, i) => (
          ...
          <VictoryStack>
            <VictoryBar
              style={{
                data: {
                  fill: ({ active }) =>
                    active ? mainColor.highlight : mainColor.base,
                  width: 10,
                },
              }}
              data={data}
